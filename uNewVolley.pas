...
type
  ...
  public
    ...
    Vipusk : Integer;  // Текущий выпуск
    BeginDate : TDate; // Начальная дата расчета
    EndDate : TDate; // Конечная дата расчета
    KoefCompens : Double; // Коэф. компенсации
    WithNorma : Integer;
    WithGrouppa : Integer;
    procedure DoWork;
  end;
...
function TfNewVolley.DoMnogitel(Value : Double; Y : Integer) : Double;
var
  MyField : TField;
begin
  Result := Value;
  if (tOtborsCodeIngrid.Value = 64) and (Result < 0) then  // Если PH
    Result := -Result;
...
procedure TfNewVolley.DoEmptyTables;
begin
  tPlata.EmptyTable;
  tTemp4.EmptyTable;
end;

procedure TfNewVolley.DoOpenTables;
begin
...
procedure TfNewVolley.DoWork;
var
  Y1, Y2, M1, M2, D1, D2 : Word;
  I, J, S : Integer;
  TempDate : TDate;
  Temp_Phakt, Temp_Limit : Double;
  NormaD, CountDate : TDate;
        ...
        begin
          ...
          begin
            ...
            if tIngrids.FindKey([tOtborsCodeIngrid.Value]) then
              tPlataC_LIMIT.Value := DoMnogitel(tIngridsNorma.Value, I);
...
//              tPlataPH_LIMIT_STR.Value := Format('%g - %g', [tVipusksPH_MIN.Value, tVipusksPH_MAX.Value]);
            ...
            if tOtborsCodeIngrid.Value = 64 then        // Если PH
            begin
//              if (not tVipusksPH_Min.IsNull) and (not tVipusksPH_Max.IsNull) then
                if (tPlataC_PHAKT.Value < 5) or (tPlataC_PHAKT.Value > 10) then
                begin
                  tPlata.Post;
                  CalcPlata;
                end
...
//              ShowMessage('Жиры');
              ...
              if (tPlataC_LIMIT.IsNull and (tPlataCodeIngrid.AsInteger <> 82) and (tPlataCodeIngrid.AsInteger <> 108)) or         // 82 - это проверка на Хром+6
                ((tPlataC_LIMIT.Value = 0.0) and (tPlataCodeIngrid.AsInteger <> 82) and (tPlataCodeIngrid.AsInteger <> 108)) or // 108 - это проверка на жиры
                (tPlataC_PHAKT.Value <= tPlataC_LIMIT.Value) then
                  tPlata.Cancel
              else
              begin
                if tPlataC_PHAKT.Value > tPlataC_LIMIT.Value * 20 then
                begin
                  tPlata.Post;
                  CalcPlata;
                end
                ...
                begin
                  if tPlataCodeIngrid.AsInteger <> 82 then    // Хром+6
                    tPlata.Cancel
                  else
                  begin
...
// Наконец-то, печатаем...
    ...
    ppLabel7.Caption := 'для ' + tWorksWorks.Value + ', ' + tVipusksOtbor.Value;
    ppLabel8.Caption := 'за период с '+DateToStr(CountDate)+' до '+DateToStr(EndDate);
...
{
procedure TfNewVolley.OldDoWork;
var
  Y1, Y2, M1, M2, D1, D2 : Word;
  I, J, S : Integer;
  TempDate : TDate;
...
procedure TfNewVolley.FillVolumes;
var
  TempDate, Dt  : TDate;
  MyY, MyM, MyD : Word;
  NormaD : TDate;
...
begin
  TempDate := BeginDate;
  Dt := EncodeDate(YearOf(BeginDate), MonthOf(BeginDate), 1);
//  Dt := BeginDate;
  mmm := 0;
...
procedure TfNewVolley.tTemp4CalcFields(DataSet: TDataSet);
begin
...
function TfNewVolley.GetV_Phakt(V : Integer; Y : Integer; D : TDate) : Double;
begin
  Result := 0.0;
  tTemp4.First;
  while not tTemp4.Eof do
...
function TfNewVolley.GetV_Limit(V : Integer; Y : Integer; D : TDate) : Double;
begin
  Result := 0.0;
  tTemp4.First;
  while not tTemp4.Eof do
...
procedure TfNewVolley.CalcPlata;
var
  PH_MAX, PH_MIN : Double;
begin
  tPlata.Edit;
  if tPlataCodeIngrid.Value <> 64 then  // Если не PH
  begin
...
procedure TfNewVolley.ppDBText7GetText(Sender: TObject;
  var Text: String);
begin
...
procedure TfNewVolley.ppVariable1Calc(Sender: TObject;
  var Value: Variant);
begin
  Value := StrToFloat(Format('%0.2f',[StrToFloat(ppDBCalc3.Value) * Procent / 100.0]));
end;

procedure TfNewVolley.ppVariable2Calc(Sender: TObject;
  var Value: Variant);
begin
//  Value := StrToFloat(Format('%0.2f',[StrToFloat(ppDBCalc5.Value) * Procent / 100.0]));
end;

procedure TfNewVolley.ppDBText12GetText(Sender: TObject;
  var Text: String);
begin
  Text := Text + ' (';
...
procedure TfNewVolley.ppLabel40GetText(Sender: TObject;
  var Text: String);
begin
  Text := DateToStr(EncodeDate(YearOf(BeginDate), MonthOf(BeginDate), 1));
end;

procedure TfNewVolley.ppLabel38GetText(Sender: TObject;
  var Text: String);
begin
  Text := DateToStr(EndDate);
end;

procedure TfNewVolley.ppLabel43GetText(Sender: TObject;
  var Text: String);
begin
  Text := Format('%0.2f руб.', [MyPlata + MyNds]);
end;

procedure TfNewVolley.ppLabel48GetText(Sender: TObject;
  var Text: String);
begin
  Text := fMain.CurOtdel;
end;

procedure TfNewVolley.ppLabel49GetText(Sender: TObject;
  var Text: String);
begin
  Text := fMain.CurBoss;
end;

procedure TfNewVolley.ppLabel53GetText(Sender: TObject;
  var Text: String);
begin
  Text := fMain.CurPhone;
end;

procedure TfNewVolley.tPlataCalcFields(DataSet: TDataSet);
var
...
procedure TfNewVolley.ppDBText1GetText(Sender: TObject;
  var Text: String);
begin
  if (tPlataCodeIngrid.Value = 67) and  // БПК5
...
procedure TfNewVolley.ppDBText5GetText(Sender: TObject;
  var Text: String);
begin
  if tPlataCodeIngrid.Value = 64 then Text := '';
end;

procedure TfNewVolley.FormCreate(Sender: TObject);
begin
  Rep.PrinterSetup.MarginBottom := fMain.CurMargin;
end;

procedure TfNewVolley.ppGroupHeaderBand5BeforePrint(Sender: TObject);
begin
...
procedure TfNewVolley.ppGroupFooterBand5BeforePrint(Sender: TObject);
begin
  if WithGrouppa = 0 then
  begin
    if WithNorma = 1 then
...
procedure TfNewVolley.ppGroupHeaderBand6BeforePrint(Sender: TObject);
begin
...
procedure TfNewVolley.RepPreviewFormCreate(Sender: TObject);
begin
  Rep.PreviewForm.WindowState := wsMaximized;
end;

procedure TfNewVolley.ppLabel16GetText(Sender: TObject;
  var Text: String);
begin
  case WithNorma of
    0 :;
...
procedure TfNewVolley.FillVolumes2;
var
  TempDate, Dt  : TDate;
  MyY, MyM, MyD : Word;
  NormaD : TDate;
...
procedure TfNewVolley.ppGroupFooterBand7BeforePrint(Sender: TObject);
begin
  if WithGrouppa = 0 then
...
procedure TfNewVolley.ppDBText23GetText(Sender: TObject;
  var Text: String);
var
  Ye : Integer;
  Y1, M1, D1 : Word;
...
begin
  try
    ...
    if tVolumes.FindKey([StrToInt(Text), tWorksCode.Value]) and not tVolumes.FieldByName('V'+IntToStr(VipuskNumber)).IsNull then
      Text := Text + ' год ('+FloatToStr(tVolumes.FieldByName('V'+IntToStr(VipuskNumber)).Value) + '%)';
    DecodeDate(BeginDate, Y1, M1, D1);
    DecodeDate(EndDate, Y2, M2, D2);
    if Y1 = Y2 then
//      Text := Text + ' с ' + DateToStr(BeginDate) + ' до ' + DateToStr(EndDate)
      Text := Text + ' с ' + DateToStr(EncodeDate(YearOf(BeginDate), MonthOf(BeginDate), 1)) + ' до ' + DateToStr(EndDate)
    else
    begin
      if Ye = Y1 then
        Text := Text + ' с ' + DateToStr(BeginDate) + ' до 01.01.' + IntToStr(Y2)
      else
        Text := Text + ' с 01.01.' + IntToStr(Y2) + ' до ' + DateToStr(EndDate);
    end;
  except
  end;
end;

procedure TfNewVolley.ppDBText22GetText(Sender: TObject;
  var Text: String);
begin
...
procedure TfNewVolley.ppDBCalc3GetText(Sender: TObject; var Text: String);
begin
...
//  MyPlata := StrToFloat(Format('%0.2f', [tPlataV_PHAKT.Value * KoefCompens * tPlataNORMA.Value * tPlata.RecordCount]));
  MyPlata := StrToFloat(Format('%0.2f', [tPlataV_PHAKT.Value * KoefCompens * tPlataNORMA.Value * tPlata.RecordCount]));
//  MyPlata := StrToFloat(Format('%0.2f', [tPlataV_PHAKT.Value * KoefCompens * tPlataNORMA.Value]));
  MyNds := StrToFloat(Format('%0.2f', [MyPlata * 20.0 / 100.0]));
  Text := 'Плата = ' + FloatToStr(tPlataV_PHAKT.Value) + ' * ' + FloatToStr(KoefCompens) + ' * ' + FloatToStr(tPlataNORMA.Value) + ' * ' + IntToStr(tPlata.RecordCount) + ' = ' + Format('%0.2f', [MyPlata]);
//  Text := 'Плата = ' + FloatToStr(tPlataV_PHAKT.Value) + ' * ' + FloatToStr(KoefCompens) + ' * ' + FloatToStr(tPlataNORMA.Value) + ' = ' + Format('%0.2f', [MyPlata]);
end;

procedure TfNewVolley.ppLabel22GetText(Sender: TObject; var Text: String);
begin
