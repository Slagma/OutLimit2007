...
type
  TResolutionType = (Before, After, Between);
  
  TfMultiSostav2 = class(TForm)
    tWorks: TTable;
  ...
  private
    ...
    MyRecNo : Integer;
    MyResolutionType : TResolutionType;
    ...
    procedure CalcPlata;
    procedure SortArrayDates(Dates : array of TDate; S : Integer);
  public
    { Public declarations }
...
var
  Procent : Double = 20.0;
  ResolutionDate : TDate; // дата отмены коэф токсичности
{ TfMultiSostav2 }

procedure TfMultiSostav2.SortArrayDates(Dates : array of TDate; S : Integer);
var
  I, J : Integer;
  TempDate : TDate;
begin
...
function TfMultiSostav2.DoMnogitel(Value : Double; Y : Integer) : Double;
var
  MyField : TField;
begin
  Result := Value;
...
procedure TfMultiSostav2.DoEmptyTables;
begin
  tPlata.EmptyTable;
  tTemp4.EmptyTable;
end;

procedure TfMultiSostav2.DoOpenTables;
begin
...
procedure TfMultiSostav2.DoWork;
var
...
// Проверяем, было ли в этот период постановление об отмене коэф. токсичности
    if ResolutionDate < BeginDate then
      MyResolutionType := Before           // Коэф. токсичности = 1
    else if ResolutionDate >= EndDate then
      MyResolutionType := After            // Коэф. токсичности из таблицы
    else
      MyResolutionType := Between;         // Коэф. токсичности меняется в указанном периоде
    ...
    NormaD := StrToDate(fMain.CurNormativs);    // Дата отмены нормативов
    ...
    if MyResolutionType = Between then
    begin
      Inc(S);
      ArrayDates[S] := ResolutionDate;
    end;

    SortArrayDates(ArrayDates, S);

    if EndDate > StrToDate('01.01.2005') then
    begin
      Inc(S);
      ArrayDates[S] := StrToDate('01.01.2005');
    end;
      ...
      // Фильтруем значения отборов для данного акта
        ...
        begin
          ...
          begin
            ...
            DecodeDate(tPlataNormaDate.Value, Y1, M1, D1);
            I := Y1;
            ...
            begin
              ...
              if (tPlataYear.AsInteger <> Y3) then
              begin
//                if (TPlataCodeIngrid.AsInteger = 95) or (tPlataCodeIngrid.AsInteger = 103) then
//                  ShowMessage('Хай');
                if tNorms.FindKey([StrToDate('01.01.'+IntToStr(tPlataYear.AsInteger)), tVipusksCode.Value, tPlataCodeIngrid.Value]) and (not tNormsNorma.IsNull) then
                  tPlataC_LIMIT.Value := DoMnogitel(tNormsNorma.Value, tPlataYear.AsInteger)
                else
                  if tNorms.FindKey([TempDate, tVipusksCode.Value, tPlataCodeIngrid.Value]) and (not tNormsNorma.IsNull) then
                    tPlataC_LIMIT.Value := DoMnogitel(tNormsNorma.Value, tPlataYear.AsInteger - 1);
              end
              else
            ...
            begin
              if tPlataNormaDate.Value >= StrToDate('06.12.2004') then
                tPlataTOKS.Value  := 1
              else
            ...
            begin
              ...
              begin
//                tGroupps.FindNearest([Work,BeginDate]);
//                tTarifs.FindNearest([BeginDate]);
//                if tTarifsDate.Value <= BeginDate then
//                  tPlataPH_TARIF.AsFloat := tTarifs.FieldByName('T'+IntToStr(tGrouppsGrouppa.Value)).AsFloat
//                else
//                begin
//                  tTarifs.Prior;
//                  if (not tTarifs.Bof) and (not tTarifs.FieldByName('T'+IntToStr(tGrouppsGrouppa.Value)).IsNull) then
//                    tPlataPH_TARIF.Value := tTarifs.FieldByName('T'+IntToStr(tGrouppsGrouppa.Value)).Value;
//                end;
            ...
            begin
              if (I <> Y1) or (J = 2) then    // Если произошел переход через год или ?
                tPlata.Cancel
              else if tPlataNormaDate.AsDateTime >= StrToDate('06.12.2004') then
                tPlata.Cancel
              else
              begin
...
//        begin
        ...
        begin
          ...
          begin
            ...
            tPlata.Insert;
            tPlataMyResolution.AsInteger := K;
...
// Наконец-то, печатаем...
    ...
    if tVipusks.RecordCount = 1 then
    begin
      ppLabel7.Caption := ppLabel7.Caption + ', ' + tVipusksOtbor.AsString;
...
// Удаляем лишние выпуски в таблице водоотведения (Temp4)
    ...
    try
      ppLabel50.Caption := fMain.ExecutorName;
      ppLabel51.Caption := fMain.ExecutorPhone;
      Rep.Print;
...
// Без смены группы в периоде
procedure TfMultiSostav2.FillVolumes;
var
  TempDate, Dt  : TDate;
  MyY, MyM, MyD : Word;
  NormaD : TDate;
...
begin
  ...
  begin
    ...
    if (TempDate = EndDate) or (MyD = 1) or (TempDate = NormaD) or (TempDate = ResolutionDate) then
    begin
      ...
      if TempDate > ResolutionDate then tTemp4Resolution.AsInteger := 1
      else                              tTemp4Resolution.AsInteger := 0;
...
//        tTemp4Limit.Value := RoundFloat(tTemp4LimitDay.Value * tTemp4Days.Value, 0);
  ...
  begin
    tTemp4.Edit;
    if tTemp4Date1.AsDateTime = BeginDate then
      tTemp4String.AsString := 'с ' + DateToStr(BeginDate) + ' до 06.12.2004';
    if tTemp4Date1.AsDateTime = StrToDate('06.12.2004') then
    begin
      if EndDate > StrToDate('01.01.2005') then
        tTemp4String.AsString := 'с 06.12.2004 до 01.01.2005'
      else
        tTemp4String.AsString := 'с 06.12.2004 до ' + DateToStr(EndDate);
    end;
    if tTemp4Date1.AsDateTime = StrToDate('01.01.2005') then
        tTemp4String.AsString := 'с 01.01.2005 до ' + DateToStr(EndDate);
    tTemp4.Post;
    tTemp4.Next;
  end;
end;

procedure TfMultiSostav2.tTemp4CalcFields(DataSet: TDataSet);
begin
...
function TfMultiSostav2.GetV_Phakt(V : Integer; Y : Integer; D : TDate) : Double;
begin
  Result := 0.0;
  tTemp4.First;
  while not tTemp4.Eof do
  begin
    if D < StrToDate('06.12.2004') then
    begin
      if (tTemp4Year.Value = Y) and (tTemp4Resolution.AsInteger = 0) and (tTemp4CodeVipusk.Value = V) then
        Result := Result + tTemp4V.Value;
    end
    else if (tTemp4Year.Value = Y) and (tTemp4Resolution.AsInteger = 1) and (tTemp4CodeVipusk.Value = V) then
      Result := Result + tTemp4V.Value;
    tTemp4.Next;
  end;
end;

function TfMultiSostav2.GetV_Limit(V : Integer; Y : Integer; D : TDate) : Double;
begin
  Result := 0.0;
  tTemp4.First;
  while not tTemp4.Eof do
...
procedure TfMultiSostav2.CalcPlata;
var
...
procedure TfMultiSostav2.ppLabel40GetText(Sender: TObject;
  var Text: String);
begin
  Text := DateToStr(BeginDate);
end;

procedure TfMultiSostav2.ppLabel38GetText(Sender: TObject;
  var Text: String);
begin
  Text := DateToStr(EndDate);
end;

procedure TfMultiSostav2.ppLabel43GetText(Sender: TObject;
  var Text: String);
begin
  Text := Format('%0.2f руб.', [StrToFloat(ppDBCalc5.Text)+StrToFloat(ppVariable2.Text)]);
end;

procedure TfMultiSostav2.ppLabel48GetText(Sender: TObject;
  var Text: String);
begin
  Text := fMain.CurOtdel;
end;

procedure TfMultiSostav2.ppLabel49GetText(Sender: TObject;
  var Text: String);
begin
  Text := fMain.CurBoss;
end;

procedure TfMultiSostav2.ppLabel53GetText(Sender: TObject;
  var Text: String);
begin
  Text := fMain.CurPhone;
end;

procedure TfMultiSostav2.tPlataCalcFields(DataSet: TDataSet);
var
...
procedure TfMultiSostav2.ppDBText1GetText(Sender: TObject;
  var Text: String);
begin
...
procedure TfMultiSostav2.ppDBText5GetText(Sender: TObject;
  var Text: String);
begin
  if (tPlataCodeIngrid.Value = 156) then Text := '';
end;

procedure TfMultiSostav2.FormCreate(Sender: TObject);
begin
  Rep.PrinterSetup.MarginBottom := fMain.CurMargin;
  ResolutionDate := StrToDate('06.12.2004');
end;

procedure TfMultiSostav2.ppGroupHeaderBand5BeforePrint(Sender: TObject);
begin
...
procedure TfMultiSostav2.ppGroupFooterBand5BeforePrint(Sender: TObject);
begin
  if WithGrouppa = 0 then
  begin
    if WithNorma = 1 then
...
procedure TfMultiSostav2.ppGroupHeaderBand6BeforePrint(Sender: TObject);
begin
//
//  if WithNorma = 1 then
...
procedure TfMultiSostav2.RepPreviewFormCreate(Sender: TObject);
begin
  Rep.PreviewForm.WindowState := wsMaximized;
end;

procedure TfMultiSostav2.ppLabel16GetText(Sender: TObject;
  var Text: String);
begin
  case WithNorma of
    0 :;
...
// Со сменой группы в указанном периоде
procedure TfMultiSostav2.FillVolumes2;
var
  TempDate, Dt  : TDate;
  MyY, MyM, MyD : Word;
  NormaD : TDate;
...
procedure TfMultiSostav2.ppGroupFooterBand7BeforePrint(Sender: TObject);
begin
  if WithGrouppa = 0 then
...
procedure TfMultiSostav2.ppDBText23GetText(Sender: TObject;
  var Text: String);
begin
  ...
  Text := Text + ', ' + tTemp4String.AsString;
end;

procedure TfMultiSostav2.ppDBText22GetText(Sender: TObject;
  var Text: String);
begin
...
procedure TfMultiSostav2.ppDBText28GetText(Sender: TObject;
  var Text: String);
begin
  if tTemp4Resolution.AsInteger = 1 then
    Text := tTemp4String.AsString;
end;

procedure TfMultiSostav2.ppGroupHeaderBand8BeforePrint(Sender: TObject);
begin
  ppGroupHeaderBand8.Visible := (tTemp4Resolution.AsInteger = 1) and (tTemp4Year.AsInteger = 2004);

end;

procedure TfMultiSostav2.ppDBText27GetText(Sender: TObject;
  var Text: String);
begin
  Text := Text + ' ';   // ???
end;

procedure TfMultiSostav2.ppGroupHeaderBand7BeforePrint(Sender: TObject);
begin
  ppGroupHeaderBand7.Visible := WithGrouppa = 1;
end;

procedure TfMultiSostav2.ppGroupHeaderBand3BeforePrint(Sender: TObject);
begin
  ppGroupHeaderBand3.Visible := tVipusks.RecordCount > 1;
end;

procedure TfMultiSostav2.ppGroupFooterBand1BeforePrint(Sender: TObject);
begin
  ppGroupFooterBand1.Visible := tVipusks.RecordCount > 1;
end;

procedure TfMultiSostav2.ppDBText26GetText(Sender: TObject;
  var Text: String);
var
  I : Integer;
  Founded : Boolean;
  ...
  LastDate : String;
begin
  if tPlataNormaDate.AsDateTime < StrToDate('06.12.2004') then
    ppLabel6.Caption := 'по постановлению администрации Красноярского края от 22.05.2001 № 349-П'
  else
    ppLabel6.Caption := 'по постановлению администрации Красноярского края от 01.12.2004 № 299-П';

  Founded := False;
  LastDate := DateToStr(EndDate);
  MyDate := StrToDate(Text);
  if MyDate < StrToDate('06.12.2004') then
    LastDate := '06.12.2004'
  else if MyDate = StrToDate('06.12.2004') then
  begin
    if EndDate > StrToDate('01.01.2005') then
      LastDate := '01.01.2005'
    else
  ...
  ppLabel47.Caption := 'с ' + Text + ' до ' + LastDate;

{
  ppLabel47.Caption := 'с ' + Text + ' до ' + DateToStr(ArrayDates[I + 1])
  for I := 1 to S do
  begin
    if ArrayDates[I] = tPlataNormaDate.AsDateTime then
    begin
      Founded := True;
      if I + 1 <= S then
        ppLabel47.Caption := 'с ' + Text + ' до ' + DateToStr(ArrayDates[I + 1])
      else
        ppLabel47.Caption := 'с ' + Text + ' до ' + DateToStr(EndDate);
      break;
    end;
  end;
  if not Founded then
