...
function TfMultiNorms2.DoMnogitel(Value : Double; Y : Integer) : Double;
var
  MyField : TField;
begin
  Result := Value;
...
procedure TfMultiNorms2.DoEmptyTables;
begin
  tPlata.EmptyTable;
  tTemp4.EmptyTable;
end;

procedure TfMultiNorms2.DoOpenTables;
begin
...
procedure TfMultiNorms2.DoWork;
var
...
//              tPlataNORMA.Value := tKoefsIngridsNorma.Value;
            ...
            if tTarifsDate.Value > BeginDate then
              tTarifs.Prior;
            tPlataNORMA.Value := tTarifs.FieldByName('T'+IntToStr(tWorksGruppa.Value)).Value;
...
procedure TfMultiNorms2.FillVolumes;
var
  MyY, MyM, MyD : Word;
  Y2, M2, D2 : Word;
...
procedure TfMultiNorms2.tTemp4CalcFields(DataSet: TDataSet);
begin
...
function TfMultiNorms2.GetV_Phakt(V : Integer; Y : Integer) : Double;
begin
  Result := 0.0;
  tTemp4.First;
  while not tTemp4.Eof do
...
function TfMultiNorms2.GetV_Limit(V : Integer; Y : Integer) : Double;
begin
  Result := 0.0;
  tTemp4.First;
  while not tTemp4.Eof do
...
procedure TfMultiNorms2.CalcPlata;
var
...
procedure TfMultiNorms2.ppDBText7GetText(Sender: TObject;
  var Text: String);
begin  if tIngrids.FindKey([tPlataCodeIngrid.Value]) and tIngridsIsPH.Value then
    Text := tPlataPH_LIMIT_STR.Value;
end;

procedure TfMultiNorms2.ppVariable1Calc(Sender: TObject;
  var Value: Variant);
begin
  Value := StrToFloat(Format('%0.2f',[StrToFloat(ppDBCalc3.Value) / 5.0]));
end;

procedure TfMultiNorms2.ppDBText3GetText(Sender: TObject;
  var Text: String);
begin
  Text := Text+' год';
end;

procedure TfMultiNorms2.ppVariable2Calc(Sender: TObject;
  var Value: Variant);
begin
  Value := StrToFloat(Format('%0.2f',[StrToFloat(ppDBCalc5.Value) / 5.0]));
end;

procedure TfMultiNorms2.ppDBText12GetText(Sender: TObject;
  var Text: String);
begin
  Text := Text + ' (';
...
procedure TfMultiNorms2.ppLabel40GetText(Sender: TObject;
  var Text: String);
begin
  Text := DateToStr(BeginDate);
end;

procedure TfMultiNorms2.ppLabel38GetText(Sender: TObject;
  var Text: String);
begin
  Text := DateToStr(EndDate);
end;

procedure TfMultiNorms2.ppLabel43GetText(Sender: TObject;
  var Text: String);
begin
  Text := Format('%0.2f руб.', [StrToFloat(ppDBCalc5.Text)+StrToFloat(ppVariable2.Text)]);
end;

procedure TfMultiNorms2.ppLabel48GetText(Sender: TObject;
  var Text: String);
begin
  Text := fMain.CurOtdel;
end;

procedure TfMultiNorms2.ppLabel49GetText(Sender: TObject;
  var Text: String);
begin
  Text := fMain.CurBoss;
end;

procedure TfMultiNorms2.ppLabel53GetText(Sender: TObject;
  var Text: String);
begin
  Text := fMain.CurPhone;
end;

procedure TfMultiNorms2.tPlataCalcFields(DataSet: TDataSet);
var
...
procedure TfMultiNorms2.ppDBText1GetText(Sender: TObject;
  var Text: String);
begin
...
procedure TfMultiNorms2.ppDBText5GetText(Sender: TObject;
  var Text: String);
begin
  if tPlataCodeIngrid.Value = 156 then Text := '';
end;

procedure TfMultiNorms2.FormCreate(Sender: TObject);
begin
  Flag := 0;
  Rep.PrinterSetup.MarginBottom := fMain.CurMargin;
end;

function TfMultiNorms2.CheckNorms(const BegD, EndD: TDate;
...
procedure TfMultiNorms2.ppDBText25GetText(Sender: TObject;
  var Text: String);
begin
...
function TfMultiNorms2.FindNorms(const BegD, EndD: TDate;
  var Norma : TDate): Boolean;
begin
  Result := CheckNorms(BegD, EndD, Norma);
end;

procedure TfMultiNorms2.ppDBText23GetText(Sender: TObject;
  var Text: String);
begin
  Text := 'c '+DateToStr(tPlataBeginDate.Value)+' по '+DateToStr(tPlataEndDate.Value);
end;
