...
// Основная работа
  ...
  begin
    ...
    // Проверяем была ли смена тарифа
...
//            tPlataC_PHAKT.Value := DoMnogitel(tOtborsValue.Value, I);
            ...
            begin
              if tIngridsIngrid.Value = 'ХПК' then
                HPK := tPlataC_PHAKT.Value;
              if tIngridsIngrid.Value = 'БПКп' then
            ...
            if fSostav.Caption = 'Состав сточных вод (новое), ' + IntToStr(fMain.CurYear) + ' год' then
            begin
              if tIngrids.FindKey([tPlataCodeIngrid.Value]) then
              begin
...
//            end
            ...
            begin
              ...
              if tIngrids.FindKey([tPlataCodeIngrid.Value]) and (tIngridsIngrid.Value = 'Соотношение ХПК/БПК5') then
              begin
...
// Наконец-то, печатаем...
    ...
    ppLabel8.Caption := 'за период с ' + DateToStr(EncodeDate(myY, myM, 1)) + ' до '+DateToStr(EndDate) + ' (дата отбора ' + DateToStr(EncodeDate(myY, myM, myD)) + ')';
...
//        tPlataPlataString.Value := '';
  ...
  begin
    if tPlataC_LIMIT.Value = 9.0 then
          tPlataPlataString.Value :=
           'Плата = [('+ Format('%0.2f', [tPlataC_PHAKT.Value]) +
                               ' - ' + Format('%0.2f', [tPlataC_LIMIT.Value])+
                               ') / ' + Format('%0.2f', [tPlataC_LIMIT.Value])+ '] *' +
          ...
          tPlataPlataString.Value :=
           'Плата = [('+ Format('%0.2f', [tPlataC_LIMIT.Value]) +
...
begin
  ...
  if (tPlataCodeIngrid.Value = BPK) then  // БПК5
    Text := 'БПК5';
  if Text = 'Соотношение ХПК/БПК5' then
    Text := Text + '( ' + tPlataPH_LIMIT_STR.Value + ')';

//  if Text = 'Соотношение ХПК/БПК5' then
//    Text := 'Соотношение ХПК/БПК5 (' + FloatToStr(HPK) + '/' + FloatToStr(BPK5) + ')';
   {
...
//    Text := Format('%0.2f руб.', [StrToFloat(ppDBCalc5.Text)]);
end;
