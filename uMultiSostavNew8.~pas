...
function TfMultiSostavNew8.DoMnogitel(Value : Double; Y : Integer) : Double;
var
  MyField : TField;
begin
  Result := Value;
...
procedure TfMultiSostavNew8.DoEmptyTables;
begin
  tPlata.EmptyTable;
  tTemp4.EmptyTable;
end;

procedure TfMultiSostavNew8.DoOpenTables;
begin
...
procedure TfMultiSostavNew8.DoWork;
var
...
procedure TfMultiSostavNew8.FillVolumes;
var
  TempDate, Dt  : TDate;
  MyY, MyM, MyD : Word;
  NormaD : TDate;
...
procedure TfMultiSostavNew8.tTemp4CalcFields(DataSet: TDataSet);
begin
...
function TfMultiSostavNew8.GetV_Phakt(V : Integer; Y : Integer; D : TDate) : Double;
begin
  Result := 0.0;
  tTemp4.First;
  while not tTemp4.Eof do
...
function TfMultiSostavNew8.GetV_Limit(V : Integer; Y : Integer; D : TDate) : Double;
begin
  Result := 0.0;
  tTemp4.First;
  while not tTemp4.Eof do
...
procedure TfMultiSostavNew8.CalcPlata;
var
      ...
      if tPlataNormaDate.Value < StrToDate(fMain.CurNormativs) then
        tPlataPlata.Value := StrToFloat(Format('%0.2f',
          [(tPlataC_PHAKT.Value - tPlataC_LIMIT.Value) *
            100.0 * tPlataNORMA.Value * 1.8 *
            tPlataV_PHAKT.Value * 1.04 / 1000000.0]))
      else
        tPlataPlata.Value := StrToFloat(Format('%0.2f',
          [(tPlataC_PHAKT.Value - tPlataC_LIMIT.Value) *
            100.0 * tPlataNORMA.Value * 1.8 *
...
procedure TfMultiSostavNew8.ppLabel40GetText(Sender: TObject;
  var Text: String);
begin
  Text := DateToStr(BeginDate);
end;

procedure TfMultiSostavNew8.ppLabel38GetText(Sender: TObject;
  var Text: String);
begin
  Text := DateToStr(EndDate);
end;

procedure TfMultiSostavNew8.ppLabel43GetText(Sender: TObject;
  var Text: String);
begin
  Text := Format('%0.2f руб.', [StrToFloat(ppDBCalc5.Text)+StrToFloat(ppVariable2.Text)]);
end;

procedure TfMultiSostavNew8.ppLabel48GetText(Sender: TObject;
  var Text: String);
begin
  Text := fMain.CurOtdel;
end;

procedure TfMultiSostavNew8.ppLabel49GetText(Sender: TObject;
  var Text: String);
begin
  Text := fMain.CurBoss;
end;

procedure TfMultiSostavNew8.ppLabel53GetText(Sender: TObject;
  var Text: String);
begin
  Text := fMain.CurPhone;
end;

procedure TfMultiSostavNew8.tPlataCalcFields(DataSet: TDataSet);
var
        ...
        if tPlataNormaDate.Value < StrToDate(fMain.CurNormativs) then
          tPlataPlataString.Value :=
           'Плата = ('+ Format('%0.3f', [tPlataC_PHAKT.Value]) +
                               ' - '+Format('%0.3f', [tPlataC_LIMIT.Value])+') * 100 * '+
                               Format('%g', [tPlataNORMA.Value]) + ' * 1.8 * '+
                               Format('%g', [tPlataV_PHAKT.Value])+ ' * 1.04'+ ' = '+
...
procedure TfMultiSostavNew8.ppDBText1GetText(Sender: TObject;
  var Text: String);
begin
...
procedure TfMultiSostavNew8.ppDBText5GetText(Sender: TObject;
  var Text: String);
begin
  if tPlataCodeIngrid.Value = PH then Text := '';
end;

procedure TfMultiSostavNew8.FormCreate(Sender: TObject);
begin
  Rep.PrinterSetup.MarginBottom := fMain.CurMargin;
end;

procedure TfMultiSostavNew8.ppGroupHeaderBand5BeforePrint(Sender: TObject);
begin
...
procedure TfMultiSostavNew8.ppGroupFooterBand5BeforePrint(Sender: TObject);
begin
  if WithGrouppa = 0 then
  begin
    if WithNorma = 1 then
...
procedure TfMultiSostavNew8.ppGroupHeaderBand6BeforePrint(Sender: TObject);
begin
...
procedure TfMultiSostavNew8.RepPreviewFormCreate(Sender: TObject);
begin
  Rep.PreviewForm.WindowState := wsMaximized;
end;

procedure TfMultiSostavNew8.ppLabel16GetText(Sender: TObject;
  var Text: String);
begin
//  case WithNorma of
//    0 :;
...
procedure TfMultiSostavNew8.FillVolumes2;
var
  TempDate, Dt  : TDate;
  MyY, MyM, MyD : Word;
  NormaD : TDate;
...
procedure TfMultiSostavNew8.ppGroupFooterBand7BeforePrint(Sender: TObject);
begin
  if WithGrouppa = 0 then
...
procedure TfMultiSostavNew8.ppDBText23GetText(Sender: TObject;
  var Text: String);
begin
...
procedure TfMultiSostavNew8.ppDBText22GetText(Sender: TObject;
  var Text: String);
begin
