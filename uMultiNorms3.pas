...
type
  TfMultiNorms3 = class(TForm)
    ...
    qPlata: TQuery;
    Label15: TLabel;
    tKoefsIngridsChanged: TDateField;
    tKoefsIngridsOrder: TIntegerField;
    tKoefsIngridsNormaLeft: TFloatField;
    tKoefsIngridsIndex: TFloatField;
    ppShape9: TppShape;
    ppDBCalc6: TppDBCalc;
    ppLabel17: TppLabel;
    ppLabel45: TppLabel;
    ...
    tNewNormsNewName2: TStringField;
    ppLabel3: TppLabel;
    ppLine3: TppLine;
    ppLabel20: TppLabel;
    ppLine7: TppLine;
...
var
  MyProc : Double = 18.0;
  PH : Integer = 64;    // Для Ларисы Васильевны
  BPK : Integer = 67;   // Для Ларисы Васильевны
//  PH : Integer = 178;    // Для Марии
//  BPK : Integer = 143;    // Для Марии
{$R *.DFM}

{ TfMultiNorms3 }

function TfMultiNorms3.DoMnogitel(Value : Double; Y : Integer) : Double;
var
  MyField : TField;
begin
  Result := Value;
...
procedure TfMultiNorms3.DoEmptyTables;
begin
  tPlata.EmptyTable;
  tTemp4.EmptyTable;
end;

procedure TfMultiNorms3.DoOpenTables;
begin
...
procedure TfMultiNorms3.DoWork;
var
...
// Основная работа
  ...
  begin
    ...
    while tVipusksCode.AsInteger <> MyVipusk do
      tVipusks.Next;
//    if not tVipusks.FindKey([MyVipusk]) then ShowMessage('Не нашел выпуска');
...
// Определяем, был ли данный ингредиент до смены нормы. Если был то продолжаем, иначе - отмена
...
//              qPlata.ParamByName('MyYear').Value := YearOf(BeginDate);
            ...
            if (I = 1) and (tOtborsCodeIngrid.Value = PH) then
            begin
              tOtbors.Next;
              Continue;
            end;
...
//              tPlataTOKS.Value  := tKoefsIngridsToks.Value;
              ...
              tPlataPH_INDEX.Value   := tKoefsIngridsIndex.Value;
            end;
...
// Наконец-то, печатаем...
    ...
    ppLabel7.Caption := 'для '+tWorksWorks.Value + ', ' + tPlataVipusk.AsString;
...
procedure TfMultiNorms3.FillVolumes;
var
  MyY, MyM, MyD : Word;
  Y2, M2, D2 : Word;
...
begin
  ...
  begin
    // Что-то делаем
    MyD1 := DateToStr(MyBeg);
    MyD2 := DateToStr(MyEnd);
    if CheckNorms(MyBeg, MyEnd, NewEnd) then
    begin
      MyNew := DateToStr(NewEnd);
      Inc(CountCheck);
      if MyBeg <> NewEnd then MyEnd := NewEnd;
      if CountCheck = 2 then Inc(MyKey);
    end;
    ND := DateToStr(NormDate);
    if ((MonthOf(NormDate) = 1) and (DayOf(NormDate) = 1) and (MonthOf(MyBeg) = 1) and (DayOf(MyBeg) = 1)) or
     (MyBeg = NormDate) then
    begin
...
procedure TfMultiNorms3.tTemp4CalcFields(DataSet: TDataSet);
begin
...
function TfMultiNorms3.GetV_Phakt(V : Integer; Y : Integer) : Double;
begin
  Result := 0.0;
  tTemp4.First;
  while not tTemp4.Eof do
...
function TfMultiNorms3.GetV_Limit(V : Integer; Y : Integer) : Double;
begin
  Result := 0.0;
  tTemp4.First;
  while not tTemp4.Eof do
...
procedure TfMultiNorms3.CalcPlata;
var
...
procedure TfMultiNorms3.ppDBText7GetText(Sender: TObject;
  var Text: String);
begin  if tIngrids.FindKey([tPlataCodeIngrid.Value]) and tIngridsIsPH.Value then
    Text := tPlataPH_LIMIT_STR.Value;
end;

procedure TfMultiNorms3.ppDBText3GetText(Sender: TObject;
  var Text: String);
begin
  Text := Text+' год';
end;

procedure TfMultiNorms3.ppVariable2Calc(Sender: TObject;
  var Value: Variant);
begin
  Value := StrToFloat(Format('%0.2f',[StrToFloat(ppDBCalc5.Value) * MyProc / 100.0]));
end;

procedure TfMultiNorms3.ppDBText12GetText(Sender: TObject;
  var Text: String);
begin
  Text := Text + ' (';
...
procedure TfMultiNorms3.ppLabel40GetText(Sender: TObject;
  var Text: String);
begin
  Text := DateToStr(BeginDate);
end;

procedure TfMultiNorms3.ppLabel38GetText(Sender: TObject;
  var Text: String);
begin
  Text := DateToStr(EndDate);
end;

procedure TfMultiNorms3.ppLabel43GetText(Sender: TObject;
  var Text: String);
begin
  Text := Format('%0.2f руб.', [StrToFloat(ppDBCalc5.Text)+StrToFloat(ppVariable2.Text)]);
end;

procedure TfMultiNorms3.ppLabel48GetText(Sender: TObject;
  var Text: String);
begin
  Text := fMain.CurOtdel;
end;

procedure TfMultiNorms3.ppLabel49GetText(Sender: TObject;
  var Text: String);
begin
  Text := fMain.CurBoss;
end;

procedure TfMultiNorms3.ppLabel53GetText(Sender: TObject;
  var Text: String);
begin
  Text := fMain.CurPhone;
end;

procedure TfMultiNorms3.tPlataCalcFields(DataSet: TDataSet);
var
...
procedure TfMultiNorms3.ppDBText1GetText(Sender: TObject;
  var Text: String);
begin
  if tNewNorms.FindKey([tPlataCodeIngrid.AsInteger]) then
  begin
    if (tPlataBeginDate.AsDateTime < StrToDate('04.08.2009')) then
      Text := tNewNormsNewName.AsString
    else
      Text := tNewNormsNewName2.AsString;
  end;
...
procedure TfMultiNorms3.ppDBText5GetText(Sender: TObject;
  var Text: String);
begin
  if tPlataCodeIngrid.Value = PH then Text := '';
end;

procedure TfMultiNorms3.FormCreate(Sender: TObject);
begin
  Flag := 1;
  Rep.PrinterSetup.MarginBottom := fMain.CurMargin;
end;

function TfMultiNorms3.CheckNorms(const BegD, EndD: TDate;
...
procedure TfMultiNorms3.ppDBText25GetText(Sender: TObject;
  var Text: String);
begin
  Text := 'c '+DateToStr(tTemp4BeginDate.Value)+' до '+DateToStr(tTemp4EndDate.Value)+
          ' ['+IntToStr(tTemp4Year.Value)+' год ('+FloatToStr(tTemp4VipuskVol.Value)+'%)]';
end;

function TfMultiNorms3.FindNorms(const BegD, EndD: TDate;
  var Norma : TDate): Boolean;
begin
  Result := CheckNorms(BegD, EndD, Norma);
end;

procedure TfMultiNorms3.ppDBText23GetText(Sender: TObject;
  var Text: String);
begin
  Text := 'c '+DateToStr(tPlataBeginDate.Value)+' до '+DateToStr(tPlataEndDate.Value);
end;

procedure TfMultiNorms3.ppDBText20GetText(Sender: TObject;
  var Text: String);
begin
  Text := FloatToStr(tTemp4Works_Limit.Value / tTemp4DaysToMonth.AsFloat);
end;

procedure TfMultiNorms3.ppVariable3Calc(Sender: TObject;
  var Value: Variant);
begin
  Value := StrToFloat(Format('%0.2f',[StrToFloat(ppDBCalc6.Value) * MyProc / 100.0]));
end;
