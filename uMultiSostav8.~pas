...
type
  TfMultiSostav8 = class(TForm)
    ...
    tPlataMyResolution: TIntegerField;
    tPlataGroupp: TIntegerField;
    tPlataKv: TFloatField;
    tPlataKi: TFloatField;
    ...
    Query1CodeVipusk: TIntegerField;
    Query1Year: TIntegerField;
    Query1Order: TIntegerField;
    Query1CodeIngrid: TIntegerField;
    Query1C_PHAKT: TFloatField;
    Query1V_PHAKT: TFloatField;
    Query1M_PHAKT: TFloatField;
    Query1C_LIMIT: TFloatField;
    Query1V_LIMIT: TFloatField;
    Query1M_LIMIT: TFloatField;
    Query1NORMA: TFloatField;
    Query1TOKS: TFloatField;
    Query1PH_LIMIT_STR: TStringField;
    Query1PH_TARIF: TFloatField;
    Query1PH_INDEX: TFloatField;
    Query1PH_ECOLOGY: TFloatField;
    Query1PH_COMPENS: TFloatField;
    Query1PLATA: TFloatField;
    Query1Key: TIntegerField;
    Query1BeginDate: TDateField;
    Query1EndDate: TDateField;
    Query1NormaDate: TDateField;
    Query1MyResolution: TIntegerField;
    Query1INDEX: TFloatField;
    Query1Groupp: TIntegerField;
    Query1Kv: TFloatField;
    Query1Ki: TFloatField;
    ppShape16: TppShape;
    ppShape18: TppShape;
    ppLabel19: TppLabel;
    ppDBText11: TppDBText;
    Query2: TQuery;
    Query2sumki: TFloatField;
    tKoefsIngridsNormaMax: TFloatField;
    tKoefsIngridsKv: TFloatField;
...
function TfMultiSostav8.DoMnogitel(Value : Double; Y : Integer) : Double;
var
  MyField : TField;
begin
  Result := Value;
  if (tOtborsCodeIngrid.Value = PH) and (Result < 0) then  // Если PH
    Result := -Result;
...
procedure TfMultiSostav8.DoEmptyTables;
begin
  tPlata.EmptyTable;
  tTemp4.EmptyTable;
end;

procedure TfMultiSostav8.DoOpenTables;
begin
...
procedure TfMultiSostav8.DoWork;
var
      ...
      // Фильтруем значения отборов для данного акта
        ...
        begin
          ...
          begin
            ...
            begin
              tPlataKv.Value := fDm.tKoefIngridsKv.Value;
              tPlataC_LIMIT.Value := fDm.tKoefIngridsNormaMax.Value;
...
//                  end;
            ...
            if tIngrids.FindKey([tOtborsCodeIngrid.Value]) then
            begin
              tPlataGroupp.Value := tIngridsGroupp.Value;
//              tPlataKv.Value := tKoefsIngridsKv.Value;
//              tPlataC_LIMIT.Value := tKoefsIngridsNormaMax.Value;
//              tPlataKv.Value := tIngridsKoefV.Value;
//              tPlataC_LIMIT.Value := tIngridsNormaMax.Value;

              if not tPlataC_LIMIT.IsNull and (tPlataC_LIMIT.Value <> 0) then
              begin
                tPlataKi.AsFloat := StrToFloat(Format('%0.2f', [(tPlataC_PHAKT.Value - tPlataC_LIMIT.Value) / tPlataC_LIMIT.Value * tPlataKv.Value]));
              end
              else
                tPlataKi.AsFloat := 0;

              if tIngridsIsPH.Value = true then
              begin
                if ((tPlataC_PHAKT.Value > 5.5) and (tPlataC_PHAKT.Value < 6)) or
                   ((tPlataC_PHAKT.Value > 9.0) and (tPlataC_PHAKT.Value < 10.0)) then
                  tPlataKv.Value := 1
                else if (tPlataC_PHAKT.Value >= 10.0) and (tPlataC_PHAKT.Value < 11.0) then
                  tPlataKv.Value := 2
                else if ((tPlataC_PHAKT.Value > 5.0) and (tPlataC_PHAKT.Value <= 5.5)) or
                        ((tPlataC_PHAKT.Value >= 11.0) and (tPlataC_PHAKT.Value <= 12.0))  then
                  tPlataKv.Value := 3
                else
                  tPlataKv.Value := 5;
                tPlataKi.Value := tPlataKv.Value;
                tPlataPLATA.Value := tPlataKi.Value * tPlataNORMA.Value * tPlataV_PHAKT.Value;
              end;
              if tIngridsIngrid.Value = 'Температура' then
              begin
                if (tPlataC_PHAKT.Value > 40) and (tPlataC_PHAKT.Value < 50) then
                  tPlataKv.Value := 0.5
                else if (tPlataC_PHAKT.Value >= 50) and (tPlataC_PHAKT.Value < 60) then
                  tPlataKv.Value := 1.0
                else if (tPlataC_PHAKT.Value >= 60) and (tPlataC_PHAKT.Value < 70) then
                  tPlataKv.Value := 2.0
                else
                  tPlataKv.Value := 3.0;
                tPlataKi.Value := (tPlataC_PHAKT.Value - tPlataC_LIMIT.Value) / tPlataC_LIMIT.Value * tPlataKv.Value;
                tPlataPLATA.Value := tPlataKi.Value * tPlataNORMA.Value * tPlataV_PHAKT.Value;
              end;
              if tIngridsIngrid.Value = 'Жиры' then
              begin
                tPlataPLATA.Value := tPlataKi.Value * tPlataNORMA.Value * tPlataV_PHAKT.Value;
              end;
...
//              end;
            ...
            if (tOtborsCodeIngrid.Value = PH) and (tOtborsValue.Value >= 6.0) and (tOtborsValue.Value <= 9.0) then        // Если PH
            begin
              tPlata.Cancel;
...
//              end;
            ...
            else if (tOtborsCodeIngrid.Value <> PH) and ((tPlataC_LIMIT.AsFloat * 100.0) < tPlataC_PHAKT.AsFloat) then
            begin
              tPlata.Cancel;
            end
            else
            begin
              if (tOtborsCodeIngrid.Value <> PH) and
              (tPlataC_LIMIT.IsNull or (tPlataC_LIMIT.Value = 0.0) or
                (tPlataC_PHAKT.Value <= tPlataC_LIMIT.Value)) then
                  tPlata.Cancel
              else
              begin
//                tPlataKi.Value := tPlata
                tPlata.Post;
//                CalcPlata;
              end;
            end;
...
// ГРУППА 1
      Query1.SQL.Clear;
      Query1.SQL.Add('update plata set plata = 0.0 where groupp <> 0 and codevipusk = ' + IntToStr(tVipusksCode.Value) + ' and plata."Year" = ' + IntToStr(I));
      Query1.ExecSQL;

      Query1.SQL.Clear;
      Query1.SQL.Add('select * from plata where codevipusk = ' + IntToStr(tVipusksCode.Value) + ' and groupp = 1 and plata."Year" = ' + IntToStr(I) + ' order by ki desc');
      Query1.Open;
      Query1.First;
      CodeMaxKi := Query1Code.Value;
      PlataMaxKi := Query1Ki.Value * Query1NORMA.Value * Query1V_PHAKT.Value;
      Query1.Close;

      Query1.SQL.Clear;
      QueryStr := 'update plata set plata = ' + Format('%0.2f', [PlataMaxKi]) + ' where code = ' + IntToStr(CodeMaxKi);
      QueryStr := StringReplace(QueryStr, ',', '.', [rfReplaceAll]);
      Query1.SQL.Add(QueryStr);
      Query1. ExecSQL;
// ГРУППА 3
      Query1.SQL.Clear;
      Query1.SQL.Add('select * from plata where codevipusk = ' + IntToStr(tVipusksCode.Value) + ' and groupp = 3 and plata."Year" = ' + IntToStr(I) + ' order by ki desc');
      Query1.Open;
      Query1.First;
      CodeMaxKi := Query1Code.Value;
      PlataMaxKi := Query1Ki.Value * Query1NORMA.Value * Query1V_PHAKT.Value;
...
// ГРУППА 5
      Query1.SQL.Clear;
      Query1.SQL.Add('select * from plata where codevipusk = ' + IntToStr(tVipusksCode.Value) + ' and groupp = 5 and plata."Year" = ' + IntToStr(I) + ' order by ki desc');
      Query1.Open;
      Query1.First;
      CodeMaxKi := Query1Code.Value;
      PlataMaxKi := Query1Ki.Value * Query1NORMA.Value * Query1V_PHAKT.Value;
...
// ГРУППА 2
      Query1.SQL.Clear;
      Query1.SQL.Add('select * from plata where codevipusk = ' + IntToStr(tVipusksCode.Value) + ' and groupp = 2 and plata."Year" = ' + IntToStr(I));
      Query1.Open;
      Query1.First;
      CodeMaxKi := Query1Code.Value;
      QueryT := Query1NORMA.Value;
      QueryQ := Query1V_PHAKT.Value;
      Query1.Close;
      Query2.SQL.Clear;
      QueryStr := 'select sum(ki) as sumki from plata where codevipusk = ' + IntToStr(tVipusksCode.Value) + ' and groupp = 2 and plata."Year" = ' + IntToStr(I);
      Query2.SQL.Add(QueryStr);
      Query2.Open;
      Query2.First;
      PlataMaxKi := Query2sumki.Value * QueryT * QueryQ;
      Query2.Close;
...
// ГРУППА 4
      Query1.SQL.Clear;
      Query1.SQL.Add('select * from plata where codevipusk = ' + IntToStr(tVipusksCode.Value) + ' and groupp = 4 and plata."Year" = ' + IntToStr(I));
      Query1.Open;
      Query1.First;
      CodeMaxKi := Query1Code.Value;
      QueryT := Query1NORMA.Value;
      ...
      Query1.Close;
      Query2.SQL.Clear;
      QueryStr := 'select sum(ki) as sumki from plata where codevipusk = ' + IntToStr(tVipusksCode.Value) + ' and groupp = 4 and plata."Year" = ' + IntToStr(I);
      Query2.SQL.Add(QueryStr);
      Query2.Open;
      Query2.First;
      PlataMaxKi := Query2sumki.Value * QueryT * QueryQ;
...
// ГРУППА 1
      Query1.SQL.Clear;
      Query1.SQL.Add('update plata set plata = 0.0 where groupp <> 0 and codevipusk = ' + IntToStr(tVipusksCode.Value));
      Query1.ExecSQL;

      Query1.SQL.Clear;
      Query1.SQL.Add('select * from plata where codevipusk = ' + IntToStr(tVipusksCode.Value) + ' and groupp = 1 order by ki desc');
      Query1.Open;
      Query1.First;
      CodeMaxKi := Query1Code.Value;
      PlataMaxKi := Query1Ki.Value * Query1NORMA.Value * Query1V_PHAKT.Value;
...
// ГРУППА 3
      Query1.SQL.Clear;
      Query1.SQL.Add('select * from plata where codevipusk = ' + IntToStr(tVipusksCode.Value) + ' and groupp = 3 order by ki desc');
      Query1.Open;
      Query1.First;
      CodeMaxKi := Query1Code.Value;
      PlataMaxKi := Query1Ki.Value * Query1NORMA.Value * Query1V_PHAKT.Value;
...
// ГРУППА 5
      Query1.SQL.Clear;
      Query1.SQL.Add('select * from plata where codevipusk = ' + IntToStr(tVipusksCode.Value) + ' and groupp = 5 order by ki desc');
      Query1.Open;
      Query1.First;
      CodeMaxKi := Query1Code.Value;
      PlataMaxKi := Query1Ki.Value * Query1NORMA.Value * Query1V_PHAKT.Value;
...
// ГРУППА 2
      Query1.SQL.Clear;
      Query1.SQL.Add('select * from plata where codevipusk = ' + IntToStr(tVipusksCode.Value) + ' and groupp = 2');
      Query1.Open;
      Query1.First;
      CodeMaxKi := Query1Code.Value;
      QueryT := Query1NORMA.Value;
      ...
      Query1.Close;
      Query2.SQL.Clear;
      QueryStr := 'select sum(ki) as sumki from plata where codevipusk = ' + IntToStr(tVipusksCode.Value) + ' and groupp = 2';
      Query2.SQL.Add(QueryStr);
      Query2.Open;
      Query2.First;
      PlataMaxKi := Query2sumki.Value * QueryT * QueryQ;
...
// ГРУППА 4
      Query1.SQL.Clear;
      Query1.SQL.Add('select * from plata where codevipusk = ' + IntToStr(tVipusksCode.Value) + ' and groupp = 4');
      Query1.Open;
      Query1.First;
      CodeMaxKi := Query1Code.Value;
      QueryT := Query1NORMA.Value;
      ...
      Query1.Close;
      Query2.SQL.Clear;
      QueryStr := 'select sum(ki) as sumki from plata where codevipusk = ' + IntToStr(tVipusksCode.Value) + ' and groupp = 4';
      Query2.SQL.Add(QueryStr);
      Query2.Open;
      Query2.First;
      PlataMaxKi := Query2sumki.Value * QueryT * QueryQ;
...
//    tPlata.IndexFieldNames := 'CodeVipusk;Year;NormaDate;Order';
    tPlata.IndexFieldNames := 'CodeVipusk;Year;NormaDate;Groupp';
    ppLabel7.Caption := 'для '+tWorksWorks.Value;
    ppLabel8.Caption := 'за период с '+DateToStr(BeginDateOtbor)+' до '+DateToStr(EndDate) + ' (дата отбора ' + DateToStr(BeginDate) + ')';
...
//        ppLabel6.Caption := 'Красноярского края от 23.08.2016г. № 422-П';
...
procedure TfMultiSostav8.FillVolumes;
var
  TempDate, Dt  : TDate;
  MyY, MyM, MyD : Word;
  NormaD : TDate;
...
procedure TfMultiSostav8.tTemp4CalcFields(DataSet: TDataSet);
begin
...
function TfMultiSostav8.GetV_Phakt(V : Integer; Y : Integer; D : TDate) : Double;
begin
  Result := 0.0;
  tTemp4.First;
  while not tTemp4.Eof do
...
function TfMultiSostav8.GetV_Limit(V : Integer; Y : Integer; D : TDate) : Double;
begin
  Result := 0.0;
  tTemp4.First;
  while not tTemp4.Eof do
...
procedure TfMultiSostav8.CalcPlata;
var
  PH_MAX, PH_MIN : Double;
begin
  tPlata.Edit;
//  if tPlataCodeIngrid.Value <> BPK then  // Если не PH
  if tPlataCodeIngrid.Value <> PH then  // Если не PH
  begin
    ...
    begin
      if tPlataNormaDate.Value < StrToDate(fMain.CurNormativs) then
        tPlataPlata.Value := StrToFloat(Format('%0.2f',
          [(tPlataC_PHAKT.Value - tPlataC_LIMIT.Value) *
            5.0 * tPlataNORMA.Value * 1.8 *
            tPlataV_PHAKT.Value / 1000000.0]))
      else
        tPlataPlata.Value := StrToFloat(Format('%0.2f',
          [(tPlataC_PHAKT.Value - tPlataC_LIMIT.Value) *
            5.0 * tPlataNORMA.Value * 1.8 *
  ...
  begin
    try
      ...
      tPlataPlata.Value := StrToFloat(Format('%0.2f',
        [(PH_MAX - PH_MIN) * tPlataPH_TARIF.Value * 1.8]));
    except
      tPlataPlata.Value := 0.0;
    end;
  end;
...
procedure TfMultiSostav8.ppLabel40GetText(Sender: TObject;
  var Text: String);
begin
  Text := DateToStr(BeginDate);
end;

procedure TfMultiSostav8.ppLabel38GetText(Sender: TObject;
  var Text: String);
begin
  Text := DateToStr(EndDate);
end;

procedure TfMultiSostav8.ppLabel43GetText(Sender: TObject;
  var Text: String);
begin
  Text := Format('%0.2f руб.', [StrToFloat(ppDBCalc5.Text)+StrToFloat(ppVariable2.Text)]);
end;

procedure TfMultiSostav8.ppLabel48GetText(Sender: TObject;
  var Text: String);
begin
  Text := fMain.CurOtdel;
end;

procedure TfMultiSostav8.ppLabel49GetText(Sender: TObject;
  var Text: String);
begin
  Text := fMain.CurBoss;
end;

procedure TfMultiSostav8.ppLabel53GetText(Sender: TObject;
  var Text: String);
begin
  Text := fMain.CurPhone;
end;

procedure TfMultiSostav8.tPlataCalcFields(DataSet: TDataSet);
var
  ...
  begin
    try
      ...
      begin
        if tPlataNormaDate.Value < StrToDate(fMain.CurNormativs) then
          tPlataPlataString.Value :=
           'Плата = ('+ Format('%0.3f', [tPlataC_PHAKT.Value]) +
                               ' - '+Format('%0.3f', [tPlataC_LIMIT.Value])+') * 5 * '+
                               Format('%g', [tPlataNORMA.Value]) + ' * 1.8 * '+
    ...
    if (Str_Min = '0.0') or
      ...
      tPlataPlataString.Value :=
        'Плата = ('+Str_Max+' - '+Str_Min+') * '+
                                FloatToStr(tPlataPH_TARIF.Value)+' * 1.8' +
                                ' = '+FloatToStr(tPlataPlata.Value)+ ' руб.';
  end;
end;

procedure TfMultiSostav8.ppDBText1GetText(Sender: TObject;
  var Text: String);
begin
  if tNewNorms.FindKey([tPlataCodeIngrid.AsInteger]) then
    Text := tNewNormsNewName.AsString;

  if (tPlataCodeIngrid.Value = BPK) and  // БПК5
...
procedure TfMultiSostav8.ppDBText5GetText(Sender: TObject;
  var Text: String);
begin
//  if tPlataCodeIngrid.Value = PH then Text := '';
end;

procedure TfMultiSostav8.FormCreate(Sender: TObject);
begin
  Rep.PrinterSetup.MarginBottom := fMain.CurMargin;
end;

procedure TfMultiSostav8.ppGroupHeaderBand5BeforePrint(Sender: TObject);
begin
...
procedure TfMultiSostav8.ppGroupFooterBand5BeforePrint(Sender: TObject);
begin
  if WithGrouppa = 0 then
  begin
    if WithNorma = 1 then
...
procedure TfMultiSostav8.ppGroupHeaderBand6BeforePrint(Sender: TObject);
begin
...
procedure TfMultiSostav8.RepPreviewFormCreate(Sender: TObject);
begin
  Rep.PreviewForm.WindowState := wsMaximized;
end;

procedure TfMultiSostav8.ppLabel16GetText(Sender: TObject;
  var Text: String);
begin
//  case WithNorma of
//    0 :;
...
procedure TfMultiSostav8.FillVolumes2;
var
  TempDate, Dt  : TDate;
  MyY, MyM, MyD : Word;
  NormaD : TDate;
...
procedure TfMultiSostav8.ppGroupFooterBand7BeforePrint(Sender: TObject);
begin
  if WithGrouppa = 0 then
...
procedure TfMultiSostav8.ppDBText23GetText(Sender: TObject;
  var Text: String);
begin
...
procedure TfMultiSostav8.ppDBText22GetText(Sender: TObject;
  var Text: String);
begin
...
procedure TfMultiSostav8.ppDBText6GetText(Sender: TObject;
  var Text: String);
begin
  if Text = '0' then
    Text := '-';
...
procedure TfMultiSostav8.ppDBText9GetText(Sender: TObject;
  var Text: String);
begin
  if tPlataPLATA.Value > 0 then
    ppDBText9.Font.Style := [fsBold, fsItalic]
  else
    ppDBText9.Font.Style := [];
end;

procedure TfMultiSostav8.ppDBText11GetText(Sender: TObject;
  var Text: String);
begin
  if Text = '0,00' then Text := '';
end;
