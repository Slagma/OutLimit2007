...
function TfMultiSostavNew6.DoMnogitel(Value : Double; Y : Integer) : Double;
var
  MyField : TField;
begin
  Result := Value;
...
procedure TfMultiSostavNew6.DoEmptyTables;
begin
  tPlata.EmptyTable;
  tTemp4.EmptyTable;
end;

procedure TfMultiSostavNew6.DoOpenTables;
begin
...
procedure TfMultiSostavNew6.DoWork;
var
...
//                end;
            ...
            if tOtborsCodeIngrid.Value = PH then        // Если PH
            begin
              tPlata.Cancel;
...
//        ppLabel6.Caption := 'Красноярского края от '+fMain.CurPost;
        ppLabel6.Caption := 'по постановлению Правительства РФ от 29.07.2013 № 644';
...
procedure TfMultiSostavNew6.FillVolumes;
var
  TempDate, Dt  : TDate;
  MyY, MyM, MyD : Word;
  NormaD : TDate;
...
procedure TfMultiSostavNew6.tTemp4CalcFields(DataSet: TDataSet);
begin
...
function TfMultiSostavNew6.GetV_Phakt(V : Integer; Y : Integer; D : TDate) : Double;
begin
  Result := 0.0;
  tTemp4.First;
  while not tTemp4.Eof do
...
function TfMultiSostavNew6.GetV_Limit(V : Integer; Y : Integer; D : TDate) : Double;
begin
  Result := 0.0;
  tTemp4.First;
  while not tTemp4.Eof do
...
procedure TfMultiSostavNew6.CalcPlata;
var
...
//  if tPlataCodeIngrid.Value <> BPK then  // Если не PH
  ...
  begin
    ...
    begin
      ...
      begin
          if (tPlataCodeIngrid.Value = 94) or
             (tPlataCodeIngrid.Value = 67) or
             (tPlataCodeIngrid.Value = 76) or
             (tPlataCodeIngrid.Value = 123) or
             (tPlataCodeIngrid.Value = 114) or
             (tPlataCodeIngrid.Value = 120) or
             (tPlataCodeIngrid.Value = 123) then

        tPlataPlata.Value := StrToFloat(Format('%0.2f',
          [(((tPlataC_PHAKT.Value - tPlataC_LIMIT.Value) *
          tPlataV_PHAKT.Value) / 1000000.0) *
          tPlataNORMA.Value * 100 * 1 * 1.19 * 1.1]))
        ...
        tPlataPlata.Value := StrToFloat(Format('%0.2f',
          [(((tPlataC_PHAKT.Value - tPlataC_LIMIT.Value) *
          tPlataV_PHAKT.Value) / 1000000.0) *
          tPlataNORMA.Value * 100 * 0.5 * 1.19 * 1.1]))
      end
      else
            if (tPlataCodeIngrid.Value = 94) or
             (tPlataCodeIngrid.Value = 67) or
...
procedure TfMultiSostavNew6.ppLabel40GetText(Sender: TObject;
  var Text: String);
begin
  Text := DateToStr(BeginDate);
end;

procedure TfMultiSostavNew6.ppLabel38GetText(Sender: TObject;
  var Text: String);
begin
  Text := DateToStr(EndDate);
end;

procedure TfMultiSostavNew6.ppLabel43GetText(Sender: TObject;
  var Text: String);
begin
  Text := Format('%0.2f руб.', [StrToFloat(ppDBCalc5.Text)+StrToFloat(ppVariable2.Text)]);
end;

procedure TfMultiSostavNew6.ppLabel48GetText(Sender: TObject;
  var Text: String);
begin
  Text := fMain.CurOtdel;
end;

procedure TfMultiSostavNew6.ppLabel49GetText(Sender: TObject;
  var Text: String);
begin
  Text := fMain.CurBoss;
end;

procedure TfMultiSostavNew6.ppLabel53GetText(Sender: TObject;
  var Text: String);
begin
  Text := fMain.CurPhone;
end;

procedure TfMultiSostavNew6.tPlataCalcFields(DataSet: TDataSet);
var
        ...
        begin
          ...
          tPlataPlataString.Value :=
           'Плата = ((('+ Format('%0.3f', [tPlataC_PHAKT.Value]) +
                               ' - '+Format('%0.3f', [tPlataC_LIMIT.Value])+') * ' +
                               Format('%g', [tPlataV_PHAKT.Value]) + ')/1000000) * ' +
...
procedure TfMultiSostavNew6.ppDBText1GetText(Sender: TObject;
  var Text: String);
begin
...
procedure TfMultiSostavNew6.ppDBText5GetText(Sender: TObject;
  var Text: String);
begin
  if tPlataCodeIngrid.Value = PH then Text := '';
end;

procedure TfMultiSostavNew6.FormCreate(Sender: TObject);
begin
  Rep.PrinterSetup.MarginBottom := fMain.CurMargin;
end;

procedure TfMultiSostavNew6.ppGroupHeaderBand5BeforePrint(Sender: TObject);
begin
...
procedure TfMultiSostavNew6.ppGroupFooterBand5BeforePrint(Sender: TObject);
begin
  if WithGrouppa = 0 then
  begin
    if WithNorma = 1 then
...
procedure TfMultiSostavNew6.ppGroupHeaderBand6BeforePrint(Sender: TObject);
begin
...
procedure TfMultiSostavNew6.RepPreviewFormCreate(Sender: TObject);
begin
  Rep.PreviewForm.WindowState := wsMaximized;
end;

procedure TfMultiSostavNew6.ppLabel16GetText(Sender: TObject;
  var Text: String);
begin
//  case WithNorma of
//    0 :;
...
procedure TfMultiSostavNew6.FillVolumes2;
var
  TempDate, Dt  : TDate;
  MyY, MyM, MyD : Word;
  NormaD : TDate;
...
procedure TfMultiSostavNew6.ppGroupFooterBand7BeforePrint(Sender: TObject);
begin
  if WithGrouppa = 0 then
...
procedure TfMultiSostavNew6.ppDBText23GetText(Sender: TObject;
  var Text: String);
begin
...
procedure TfMultiSostavNew6.ppDBText22GetText(Sender: TObject;
  var Text: String);
begin
